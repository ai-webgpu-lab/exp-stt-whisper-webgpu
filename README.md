...
`Whisper STT`를 단일 질문으로 분리해 검증하는 experiment 저장소입니다. 하나의 가설과 baseline에 집중하고, 결과를 재현 가능한 형식으로 남겨 이후 benchmark/app 저장소의 입력으로 사용합니다.
...
## 핵심 질문
- Whisper STT를 브라우저/WebGPU 환경에서 재현 가능한 baseline으로 만들 수 있는가
...
- Repo-specific Pages baseline copied from `repo-scaffolds/p0/exp-stt-whisper-webgpu/`
...
## 현재 baseline 상태
- Repository-specific runnable baseline active: file transcription readiness harness with deterministic segment processing, first-partial timing, and WER/CER estimation
- Generated override source: `repo-scaffolds/p0/exp-stt-whisper-webgpu/`
- Results/report scaffold is ready to promote exported JSON into `reports/raw/` and `RESULTS.md`

## GitHub Pages 운영 메모
- Pages URL: https://ai-webgpu-lab.github.io/exp-stt-whisper-webgpu/
...
- 이 저장소의 live demo는 `https://ai-webgpu-lab.github.io/exp-stt-whisper-webgpu/`입니다.
